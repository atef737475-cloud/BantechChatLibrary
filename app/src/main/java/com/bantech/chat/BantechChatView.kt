...
class BantechChatView @JvmOverloads constructor(
    ...
    private fun buildChatUrl(config: BantechChatConfig): String {
        ...
        return StringBuilder().apply {
            append("http://chat.bantech.ae/")
            append("?uuid=").append(uuid)
            append("&token=").append("${config.customerId}|${config.customerToken}")
            ...
            if (config.wsHost != "ws.bantech.ae") {
                append("&wsHost=").append(config.wsHost)
            }

            if (config.apiBaseUrl != "https://ai-chatbot.bantech.ae/api/") {
                append("&apiBaseUrl=").append(config.apiBaseUrl)
            }
        }.toString()
    }
